...
The explanation you see after grading shows exactly which sentences helped or hurt your score, and why.
    ...
    processQuery(query) {
        ...
        if (/how\s+many\s+students/.test(lowerQuery) ||
        ...
        if (/highest.*score|best.*student|top.*student|who.*score.*most/.test(lowerQuery) ||
...
• <strong>Jaccard (Word Overlap):</strong> ${topStudent.jaccard.toFixed(3)}
        ...
        if (/lowest.*score|worst.*student|bottom.*student|minimum.*score/.test(lowerQuery)) {
            const stats = this.getStatistics();
...
The student may need additional support or clarification on the concepts.
        ...
        const aboveMatch = lowerQuery.match(/(?:more than|greater than|above|>\s*)(\d+(?:\.\d+)?)/i);
        if (aboveMatch) {
        ...
        const belowMatch = lowerQuery.match(/(?:less than|below|fewer than|<\s*)(\d+(?:\.\d+)?)/i);
        if (belowMatch) {
        ...
        if (/score.*for|marks.*of|grade.*of|performance.*of|result.*of/.test(lowerQuery) ||
            /(^|\s)(who is|what|score\s+)/i.test(lowerQuery)) {
            // Extract potential student identifier
            const nameMatch = query.match(/(?:for|of|by)\s+([a-zA-Z\s]+?)(?:\s+\(|$|\?|,)/i);
            ...
            if (!searchQuery) {
                ...
                searchQuery = words.filter(w => /^[a-zA-Z]/.test(w) && w.length > 2)
                    .join(' ')
                    .replace(/[?,.]/g, '');
            }
        ...
        if (/how.*work|explain.*system|two.?stage|grading.*process|pipeline|stage 1|stage 2|rule.?based|semantic|nlp/.test(lowerQuery) ||
